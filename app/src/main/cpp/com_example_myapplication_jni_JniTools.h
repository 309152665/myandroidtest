/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_example_myapplication_jni_JniTools */

#ifndef _Included_com_example_myapplication_jni_JniTools
#define _Included_com_example_myapplication_jni_JniTools
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_example_myapplication_jni_JniTools
 * Method:    addNum
 * Signature: (II)I
 */
JNIEXPORT jint JNICALL Java_com_example_myapplication_jni_JniTools_addNum
  (JNIEnv *, jobject, jint, jint);

/*
 * Class:     com_example_myapplication_jni_JniTools
 * Method:    init
 * Signature: ()J
 */
JNIEXPORT jlong JNICALL Java_com_example_myapplication_jni_JniTools_init
  (JNIEnv *, jobject);

/*
 * Class:     com_example_myapplication_jni_JniTools
 * Method:    getPersonAge
 * Signature: (J)I
 */
JNIEXPORT jint JNICALL Java_com_example_myapplication_jni_JniTools_getPersonAge
  (JNIEnv *, jobject, jlong);

#ifdef __cplusplus
}
#endif
#endif
